...
class AVAPredictor(object):
    def __init__(
        ...
        self.model.eval()
        # Model will move to GPU lazily inside the worker process
...
class AVAPredictorWorker(object):
    ...
    def __init__(self, cfg):
        ...
        if self.ava_predictor.has_object:
            self.object_cfg = copy.deepcopy(cfg)
            self.object_cfg.detector = "yolo"
            self.coco_det = None
        else:
            self.coco_det = None
    ...
    def _compute_prediction(self):
        ...
        if self.ava_predictor.has_object and self.coco_det is None:
            self.coco_det = get_detector(self.object_cfg)
        ...
        for i in count():
            ...
            if self.task_done == True and empty_flag:
                ...
                for center_timestamp, video_size, ids in tqdm(self.timestamps[pred_num_cnt:], initial=pred_num_cnt,
                                                              total=len(self.timestamps), desc="Action Prediction"):
